...
CREATE UNIQUE INDEX idx_pa_cuenta_numero
  ON pasivo.pa_cuenta (cu_numero);
...
CREATE TABLE pasivo.pa_movimiento (
  ...
  mo_estado           BOOLEAN       NOT NULL,
  CONSTRAINT fk_movimiento_cuenta
  FOREIGN KEY (cu_id) REFERENCES pasivo.pa_cuenta (cu_id)
);

CREATE INDEX idx_pa_movimiento_fecha
  ON pasivo.pa_movimiento (mo_fecha);

CREATE INDEX idx_pa_movimiento_cliente
  ON pasivo.pa_movimiento (cl_id);