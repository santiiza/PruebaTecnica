...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponseDto> handleUnexpectedTypeException(UnexpectedTypeException ex) {
        log.error("<-| Process UnexpectedTypeException finished with error - message:{}", ex.getMessage());
        log.debug("<-| Process UnexpectedTypeException finished with error - message:{}", ex.getMessage(), ex);
        ErrorResponseDto errorResponse = new ErrorResponseDto(
                HttpStatus.INTERNAL_SERVER_ERROR.value(),
                "Error: " + ex.getMessage(),
                LocalDateTime.now()
